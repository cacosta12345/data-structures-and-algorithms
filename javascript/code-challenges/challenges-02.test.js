...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach((value,index)=>{
    let powerNumber = Math.pow(value, 3);
    newArray.push(powerNumber);
...
const addOne = (arr) => {
  let mappedArr = arr.map((value,index)=>{
    return value +1;
  });
  return mappedArr;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (const number of arr){
    let newNums =Math.pow(2, number);
    newArray.push(newNums);
  }
  return newArray;
};
...
const forEachTwoToThe = (arr) => {
  let newArray =[];
  arr.forEach((value,index)=>{
    let powerNum = Math.pow(2, value);
    newArray.push(powerNum);
  });
  return newArray;
};
