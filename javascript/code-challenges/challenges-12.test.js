...
function transformToLis(obj) {
  ...
  const listItems = pairs.map(([key, value]) => `<li>${key}: ${value}</li>`);
...
const count = (target, input) => {
  return input.reduce((totalCount, currentArray) => {
    const timesInArray = currentArray.filter(element => element === target).length;
    return totalCount + timesInArray;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((sum, currentArray) => {
    const arraySum = currentArray.reduce((acc, element) => acc + element, 0);
    return sum + arraySum;
  }, 0);
};
...
let findShortest = (data) => {
  const shortestCharacter = data.reduce((shortest, currentCharacter) => {
    const currentHeight = parseFloat(currentCharacter.height);
    if (!isNaN(currentHeight) && (isNaN(shortest.height) || currentHeight < parseFloat(shortest.height))) {
